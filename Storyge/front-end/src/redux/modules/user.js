...
export const kakaoLogin = async (code) => {
  axios({
    method: "GET",
    url: `http://${ServerURL}/oauth/callback/kakao?code=${code}&state=kakao&prompt=none`,
  })
    .then((res) => {
      console.log(res);
      ...
      window.location.href = `http://${localURL}/`; // 토큰 받았았고 로그인됐으니 화면 전환시켜줌(메인으로)
    })
    .catch((err) => {
      console.log("소셜로그인 에러", err);
      window.alert("로그인에 실패하였습니다.");
      window.location.href = `http://${localURL}/login`; // 로그인 실패하면 로그인화면으로 돌려보냄
    });
};
...
export const googleLogin = async (code) => {
  axios({
    ...
    .catch((err) => {
      console.log("소셜로그인 에러", err);
      window.alert("로그인에 실패하였습니다.");
      window.location.href = `http://${localURL}/login`;
    });
};
